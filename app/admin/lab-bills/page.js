...
export default function LabBillsPage() {
  ...
  const fetchBills = async () => {
    const res = await fetch("/api/lab-bills");
    const data = await res.json();
  ...
  const handleAddRow = () => {
    setItems((prev) => [...prev, { inventoryId: "", quantity: "", unitPrice: "" }]);
  };
  ...
  const handleCreateBill = async () => {
    ...
    try {
      ...
      alert("Lab bill created successfully");
      setPatientName("");
      setPatientPhone("");
      setPatientAge("");
      setPatientGender("");
                  ...
                  >
                    ...
                    {inventory.map((inv) => (
                      <option key={inv._id} value={inv._id}>
                        {inv.testName} ({inv.quantity} {inv.unit || ""})
                      </option>
                    ))}
                  </select>
                </div>
                                  ...
                                  <body style="font-family: Arial, sans-serif; padding: 20px; color: #111827;">
                                    <div style="display:flex; justify-content:space-between; gap:16px; border-bottom: 1px solid #e5e7eb; padding-bottom: 12px; margin-bottom: 16px;">
                                      <div>
                                        ...
                                        <div style="font-size: 12px; margin-top: 4px;">Phone: ${escapeHtml(
                                          HOSPITAL.phone
                                        )} | ${escapeHtml(HOSPITAL.email)}</div>
                                      </div>
                                      <div style="text-align:right;">
                                        <div style="font-size: 12px;">Bill ID: ${escapeHtml(
                                          b._id
                                        )}</div>
                                        <div style="font-size: 12px; margin-top: 4px;">Date: ${
                                          b.createdAt ? escapeHtml(new Date(b.createdAt).toLocaleString()) : "-"
                                        }</div>
                                      </div>
                                    </div>
                                    ...
                                    <div style="margin-bottom: 16px;">
                                      ...
                                      <table style="width:100%; border-collapse:collapse; font-size:13px;">
                                        <thead>
                                          <tr>
                                            <th style="padding:8px;border-bottom:1px solid #e5e7eb;text-align:left;">Test</th>
                                            <th style="padding:8px;border-bottom:1px solid #e5e7eb;text-align:left;">Sample</th>
                                      ...
                                      <div style="margin-top: 12px; display:flex; justify-content:flex-end;">
                                        <div style="border:1px solid #e5e7eb; padding:10px 14px; border-radius:10px;">
                                          ...
                                          <div style="font-size:18px; font-weight:bold;">₹${escapeHtml(
                                            b.total
                                          )}</div>
                                        </div>
                                      </div>
                          ...
                          <div className="p-4 sm:p-6 bg-slate-50">
                            <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 mb-4">
                              ...
                              <div className="text-sm text-slate-600 md:text-right">
                                ...
                                <div className="mt-1">{b.createdAt ? new Date(b.createdAt).toLocaleString() : "-"}</div>
                              </div>
                            </div>
                            ...
                            <div className="bg-white border border-slate-200 rounded-xl overflow-x-auto">
                              <table className="min-w-full text-sm">
                                <thead className="bg-slate-100">
                                  <tr>
                                    <th className="px-3 py-2 text-left border-b">Test</th>
                                    <th className="px-3 py-2 text-left border-b">Sample</th>
                                ...
                                <tbody>
                                  {(b.items || []).map((it, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50">
                                      <td className="px-3 py-2 border-b">{it.testName || "-"}</td>
                                      <td className="px-3 py-2 border-b">{it.sampleType || "-"}</td>
