...
export default function LabInventoryPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let res;
      if (editId) {
        res = await fetch(`/api/lab-inventory/${editId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
      } else {
        res = await fetch("/api/lab-inventory", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
    ...
    <div className="max-w-5xl mx-auto w-full">
      <div className="bg-white shadow-sm border border-slate-200 rounded-2xl p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <input
            className="border rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-400"
            placeholder="Sample type (optional)"
            value={sampleType}
          ...
          <input
            type="number"
            className="border rounded-lg p-3 focus:outline-none focus:ring-2 focus:ring-blue-400 sm:col-span-2"
      ...
      <div className="mt-6 bg-white shadow-sm border border-slate-200 rounded-2xl p-4 sm:p-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-lg font-bold text-slate-800">Inventory List</h3>
          <button
            onClick={fetchItems}
