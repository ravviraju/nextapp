...
export async function getAllLabBills() {
  try {
...
export async function createLabBill({
  ...
  try {
    ...
    const db = client.db();
    const collection = db.collection("labBills");
    const inventoryCollection = db.collection("labInventory");
    ...
    for (const it of items) {
      ...
      if (inv.quantity < qty) {
        throw new Error(
          `Insufficient stock for ${inv.testName} (available: ${inv.quantity})`
        );
      }
    ...
    for (const r of resolvedItems) {
      await inventoryCollection.updateOne(
        { _id: r.inventoryId, quantity: { $gte: r.quantity } },
    ...
    return { id: result.insertedId, total };
  } catch (error) {
